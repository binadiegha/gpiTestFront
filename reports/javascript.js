            ...
            columnDefs: [
                {
                    ...
                    render: function(data, type, full, meta) {
                        //var dfg = JSON.parse(full);
                        var vb = [];

                        //var str = "Hello, world, welcome, to, the, universe";
                        var count = occurrences(full, ","); //(full.match(/is/g) || []).length;

                        for (a = 0; a < count - 1; a++) {
                            theindex = full.indexOf(",");

                            var res = full.substring(0, theindex);
                            vb[a] = res;//str.indexOf(",");
                            full = full.substring(count);
                        ...
                        console.log(vb);
                        //var res = full;
                        //var this_code = `cutoff=`+res[0]+`&testname=`+res[1]+`&score=`+res+`&datetaken=`+res+`&publisher=`+res;
                        return `
                        <span class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                ...
                                <a class="dropdown-item" href="../psyreport/?`+`this_code`+`"><i class="la la-print"></i> Generate Report</a>
                            </div>
                        </span>
